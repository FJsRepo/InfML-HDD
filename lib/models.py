...
from nets.Proposed import resnet18, resnet34, resnet50, resnet101
...
class HorizonRegression(nn.Module):
    ...
    def loss(self,
        ...
        # calculate the true slope of the horizon
        # target_k = torch.zeros(target_left_points.shape[0], 1)
        # for i in range(target_left_points.shape[0]):
        #     target_k[i] = 288 * (target_right_points[i] - target_left_points[i])/383
        # device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        # target_k = target_k.to(device)

        # pred = [conf, pred_left_points, pred_right_points, k, b]
