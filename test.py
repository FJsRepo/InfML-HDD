...
def test(model, test_loader, exp_root, cfg, view, epoch, max_batches=None, verbose=True):
    ...
    with torch.no_grad():
        ...
        for idx, (images, labels, img_idxs) in loop:
            ...
            t0 = time()
            outputs, dist1, dist2, dist3, dist4 = model(images)
            t = time() - t0
            time_consume += t
            loss_i, loss_dict_i = criterion(outputs, dist1, dist2, dist3, dist4, labels, **criterion_parameters)
